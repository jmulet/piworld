...
module.exports = function (app) {
                ...
                fs.unlink(toDir + "QProvider.js", function () {
                    fs.unlink(toDir + "QProvider.min.js", function () {
                        generator(fromDir, toDir, function () {
                            app.db.query("UPDATE activities SET hasAct=1 WHERE id=" + id)();
                            res.send({ ok: true, path: path, log: log.join('<br>') });
                        }, function () { res.send({ ok: false, path: path, log: log.join('<br>') }); }, log);
                    });
                });
    ...
    var uploadSancions = function (req, res) {
        
        const bonificador = function (f, periodeBonificacio, onlyOne)  {
             
            const book = XLSX.readFile(f, { type: 'binary', bookType: 'biff8' });
            const sheetName1 = book.SheetNames[0];
            const sheet1 = book.Sheets[sheetName1];

            let pass = true;
            const AlumnesRaw = {};
            const AlumnesProcessed = [];
            const rows = sheet1['!rows'].length;

            for (var i = 0; i < rows; i++) {
                let cell_ref = XLSX.utils.encode_cell({ r: i, c: 0 });
                const cell = sheet1[cell_ref];               
                if (cell) {
                    ...
                    if (pass) {
                        ...
                        const nomAlumne = cell.v;
                        let list = AlumnesRaw[nomAlumne];
                        if (!list) {
                            list = [];
                            AlumnesRaw[nomAlumne] = list;
                        }
                        cell_ref = XLSX.utils.encode_cell({ r: i, c: 1 });
                        const day = sheet1[cell_ref].v;
                        cell_ref = XLSX.utils.encode_cell({ r: i, c: 4 });
                        const type = sheet1[cell_ref].v;
                        let w = 0;
                        if (type === 'Avís') {
                            w = 1;
                        } else if (type === 'Lleu') {
                        ...
                        list.push({ day: day, date: crono.parseDate(day), type: type, weight: w });
                    }
                }
            }
            ...
            for (let nom in AlumnesRaw) {
 
                let list = AlumnesRaw[nom];
                // Make sure we sort each student's list by date
                list = list.sort(function (e2, e1) {
                ...
                // La variable bonificat és el nombre d'avisos acumulats al llarg del temps
                let bonificat = list[0].weight; //Començam amb 1 o 2 avisos
                log.push(list[0].day + "[" + bonificat + "] -->  ");

                for (let i = 1; i < list.length; i++) {
                    const obj = list[i];
                    const current = obj.date;                    
                    const daysEllapsed = crono.workDaysBetween(lastDate, current);
                    let bonifica = Math.trunc( daysEllapsed / periodeBonificacio);
                    if (onlyOne) {
                        bonifica = bonifica ? 1 : 0;
                    }
                    // Com a màxim puc restar el nombre acumulat d'avisos "bonificat"
                    if (bonifica > bonificat) {
                        bonifica = bonificat;
                    }
                     
                    if (bonifica) {
                        log.push(Math.trunc(daysEllapsed) + " dies;  -" + bonifica + " avís  -->  ");
                    } else {
                        log.push(Math.trunc(daysEllapsed) + " dies  -->  ");
                    }
                    // La nova variable és l'anterior més l'avis actual - la bonificacio des de l'anterior
                    bonificat = bonificat + obj.weight - bonifica;
                    if (bonificat < 0) {
                        bonificat = 0;
                    }
                    lastDate = current;
                    log.push(obj.day + "[" + obj.weight+ "] -->  ");
                }
                // Finalment mira des de la data actual el darrer avis  
                ...
                if (bonifica) {
                    log.push("fins avui: "+ Math.trunc(daysEllapsed) + " dies;  -" + bonifica + " avís.  ");
                } else {
                    log.push("fins avui: "+ Math.trunc(daysEllapsed) + " dies ");
                }
                // La nova variable és l'anterior - la bonificacio des de l'anterior (no sumam avis perque no n'hi ha encara)
                bonificat = bonificat - bonifica;
                if (bonificat < 0) {
                    bonificat = 0;
                }
                
                AlumnesProcessed.push({ nom: nom, total: total, bonificat: bonificat, log: log });
            }


            //console.log(AlumnesProcessed)
            return AlumnesProcessed;
        };
        ...
        form.parse(req, function (err, fields, myfiles) {
            let periodeBonificacio = parseFloat(fields.periodeBonificacio) || 10.0; //dies
            let onlyOne = (fields.onlyOne === "true");
            if (err) {
                console.log(err);
                res.send([]);
                return;
            ...
            files.forEach(function (f) {
                response.push(bonificador(f.path, periodeBonificacio, onlyOne));
    ...
    app.post('/rest/fs/uploads/bonificador', uploadSancions);
};