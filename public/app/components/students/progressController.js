define([], function(){
  "use strict";  
window.pw.app.register.controller('ProgressController', ['$http', '$scope', '$rootScope', '$filter', 
    ...
    function ($http, $scope, $rootScope, $filter, $transition$, $uibModal, growl, $translate,
        ...
        $http.post("/rest/students/list",{idGroup: $scope.grpID, edit: 0}).
            ...
            var data  = r.data;
            $scope.students = data;
            if(!$scope.userID && data.length>0){
                $scope.user = data[0];                
            }
            else
            {
                //find the student from the data list
                data.forEach(function(e){
                    if(e.id==$scope.userID)
                    {
                        $scope.user = e;
                    }
                });
