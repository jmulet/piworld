...
window.pw.app.register.controller("HomeController",["HomeService","$scope","$rootScope","$state","$http","$translate","USER_ROLES","$uibModal","Modals","$window","$timeout","Auth","Session","growl","MyCacheService","$ocLazyLoad",function(f,c,d,m,p,q,k,e,n,b,a,t,l,g,u,v){c.USER_ROLES=k;f.initialize();var h;v.load("ngIdle").then(["Idle","IdleProvider",function(a,b){b.idle(window.pw.isMobile()?86400:600);b.timeout(30);a.watch();d.$on("IdleStart",function(){h=e.open({templateUrl:"app/shared/alert-dlg.html",
controller:["$scope",function(a){a.count=30;a.ok=function(){h.close(!0)};a.title="Temps d'inactivitat";a.msg="Es tancar\u00e0 la sessi\u00f3 en {{count}} segons."}]});h.result.then(function(b){b&&a.watch()},function(){a.watch()})});d.$on("IdleTimeout",function(){h&&h.close();l.logout()});d.$on("IdleEnd",function(){h.close(!0)})}]);c.user=l.getUser();c.$on("changeGrp",function(a,b){c.selectedGroup=b.group;s(!0)});c.user&&c.user.idRole!==k.guest||m.go("login");var w=u.getCache("HomeCache",!1),s=function(a){var b=
0,r=c.selectedGroup;r&&("B"===r.groupStudies[0]?b=2:2<r.groupLevel&&(b=1),w.httpCache("all","oftheday/all",{idUser:c.user.id,level:b,lang:q.use(),idGroup:r.idGroup},a).then(function(a){c.randomEqn=a.eqn;c.randomQuote=a.quote;c.challenge=a.problem}))};c.leftClass="col-md-3 col-lg-3";c.mainClass="col-md-9 col-lg-7";c.rightClass="col-md-12 col-lg-2";c.dc="";f=749<jQuery("body").width();c.tgs={mobileShow:1,welcome:f,score:f,recent:!0,assignments:!0,famouseqn:f,quote:f,problem:f,groups:0===c.user.groups.length,
...
"";d.newpwd="";d.newpwd2="";d.username=(a?"Familia de ":"")+c.user.username;d.ok=function(){t.checkPassword(d.oldpwd||"").then(function(a){a.ok?6>d.newpwd.trim().length?g.error("La longitud m\u00ednima de la contrasenya s\u00f3n 6 car\u00e0cters."):d.newpwd.trim()===c.user.email.trim()?g.error("La constrasenya ha d'esser diferent que el correu electr\u00f2nic"):(a=/[0-9]/,a.test(d.newpwd)?(a=/[a-z]/,a.test(d.newpwd)?d.newpwd!==d.newpwd2?g.error("Les contrasenyes no coincideixen."):b.close(d.newpwd):
